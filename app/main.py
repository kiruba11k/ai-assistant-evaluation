    ...
    with gr.Blocks(
        ...
        with gr.Tabs():
            ...
            with gr.Tab("🟢 Frontier Assistant (Claude)"):
                ...
                def frontier_send_fn(msg, hist, state):
                    if not config.ANTHROPIC_API_KEY:
                        hist.append((msg, " ANTHROPIC_API_KEY not set. Add it to your .env file."))
                        return hist, hist, state, ""
            ...
            with gr.Tab("⚔️ Head-to-Head"):
                ...
                h2h_send = gr.Button("⚔️ Compare Both", variant="primary")
                ...
                def h2h_send_fn(msg, oh, fh, os_, fs_):
                    ...
                    if fs_ is None:
                        if not config.ANTHROPIC_API_KEY:
                            fh.append((msg, " ANTHROPIC_API_KEY not configured"))
                            fs_ = None
                        else:
                            try:
                                fs_ = make_frontier()
